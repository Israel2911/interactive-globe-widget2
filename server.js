...
// ===
// ===
// FIXED MIDDLEWARE - PREVENTS INFINITE REDIRECTS
// ===
app.use((req, res, next) => {
  try {
    console.log('🔍 Checking for mToken in URL...');
    
    // Skip if we've already processed a token for this request
    if (req.tokenProcessed) {
      return next();
    }
    ...
    if (token) {
      console.log('🎯 Processing simple token:', token.substring(0, 50) + '...');
      
      // Mark this request as having processed a token
      req.tokenProcessed = true;
      
      // Set session data
      req.session.isLoggedIn = true;
      req.session.wixUserId = 'user-' + Date.now();
      req.session.userEmail = 'authenticated-user@example.com';
      req.session.userName = 'Authenticated User';
      req.session.authMethod = 'wix_members';
      
      console.log(`✅ Session set for authenticated user`);
      
      // Create clean URL without mToken
